...
class ComponentV2 extends Component {
  constructor(props) {
    ...
    try {
      ...
      if (qs.get('reset') === '1') sessionStorage.removeItem('rsIntelState');
      if (qs.get('intel') === '1') startOn = true;
    } catch (e) {}

    // Survive an accidental mid-demo refresh: enablement and approvals mirror
    // to sessionStorage (per-tab, so a fresh tab is still a clean rehearsal
    // reset; ?reset=1 clears explicitly).
    var saved = null;
    try { saved = JSON.parse(sessionStorage.getItem('rsIntelState') || 'null'); } catch (e) {}
    if (saved && saved.intel) startOn = true;
    ...
    this.renderVals = function () {
      ...
      // Mirror demo-critical state so a mid-demo refresh recovers (per-tab).
      try {
